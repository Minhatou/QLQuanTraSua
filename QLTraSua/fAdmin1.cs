...
namespace CoffeManager
{
    public partial class fAdmin1 : Form
    {
		...
		private void fPrint_Click(object sender, EventArgs e)
		{
			...
			exSheet.Range["F10"].ColumnWidth = 25;
			exSheet.Range["A10"].Value = "STT: ";
			exSheet.Range["B10"].Value = "Bàn số: ";
			exSheet.Range["C10"].Value = "Thành tiền: ";
			exSheet.Range["D10"].Value = "Thời gian vào: ";
			exSheet.Range["E10"].Value = "Thời gian ra: ";
			exSheet.Range["F10"].Value = "Khuyến mãi (%): ";

			int dong = 11; int tongtien = 0;
			for (int i = 0; i < fDGVBill.Rows.Count - 1; i++)
			{
				exSheet.Range["A" + (dong + i).ToString()].Value = (i + 1).ToString();
				exSheet.Range["B" + (dong + i).ToString()].Value = fDGVBill.Rows[i].Cells[0].Value.ToString();
				exSheet.Range["C" + (dong + i).ToString()].Value = fDGVBill.Rows[i].Cells[1].Value.ToString();
                tongtien += Convert.ToInt32(fDGVBill.Rows[i].Cells[1].Value);
				exSheet.Range["D" + (dong + i).ToString()].Value = fDGVBill.Rows[i].Cells[2].Value.ToString();
				exSheet.Range["E" + (dong + i).ToString()].Value = fDGVBill.Rows[i].Cells[3].Value.ToString();
				exSheet.Range["F" + (dong + i).ToString()].Value = fDGVBill.Rows[i].Cells[4].Value.ToString();
			}
